...
def _asset_root() -> Path:
    candidates = [
        ...
        Path(site.USER_BASE) / "share" / "overkill-factory",
        Path(sysconfig.get_path("data") or "") / "share" / "overkill-factory",
    ]
    return next((candidate for candidate in candidates if (candidate / "templates" / "factory-master-plan-completion.json").exists()), CODE_ROOT)
ROOT = _asset_root()
...
def _path_exists(root: Path, ref: str) -> bool:
    if ref.startswith(("command:", "github:", "release:", "issue:", "tag:", "external:")):
        return True
    return (root / ref).exists() or (REPO_ROOT / ref).exists() or (CODE_ROOT / ref).exists()
