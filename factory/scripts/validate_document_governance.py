...
CURRENT_AUTHORITY_REFS = (
    "README.md",
    "legacy-docs/concepts/factory-flow.md",
    "legacy-docs/operations/validation-and-release.md",
    "scripts/factoryctl.py",
    "schemas/",
    "tests/",
)
PUBLIC_DOC_FORBIDDEN_DIRS = (
    ROOT / "legacy-docs" / "methodology",
    ROOT / "legacy-docs" / "roadmap",
    ROOT / "legacy-docs" / "reviews",
    ROOT / "legacy-docs" / "planning",
    ROOT / "legacy-docs" / "pilots",
    ROOT / "legacy-docs" / "validation",
    ROOT / "legacy-docs" / "research",
    ROOT / "legacy-docs" / "maps",
    ROOT / "legacy-docs" / "illustrations",
)
AMBIGUITY_TERMS = (
    "still open",
    "remaining",
...
def validate_ambiguous_docs_have_status() -> list[str]:
    errors: list[str] = []
    for path in sorted((ROOT / "legacy-docs").rglob("*.md")):
        head = front_matter(path)
