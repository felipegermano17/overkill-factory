...
WORKFLOW_CATALOG_PATH = ROOT / "legacy-docs" / "factory-workflow.catalog.json"
SECURITY_MATRIX_PATH = ROOT / "legacy-docs" / "agents" / "security-specialist-matrix.md"
...
def validate_workflow_catalog_alignment(
    ...
    if not WORKFLOW_CATALOG_PATH.exists():
        return ["legacy-docs/factory-workflow.catalog.json is missing"]

    try:
        workflow = load_json(WORKFLOW_CATALOG_PATH)
    except (json.JSONDecodeError, ValueError) as exc:
        return [f"legacy-docs/factory-workflow.catalog.json is invalid: {exc}"]

    phases = workflow.get("phases", [])
    if not isinstance(phases, list):
        return ["legacy-docs/factory-workflow.catalog.json phases must be an array"]
...
def validate(
    *,
    ...
    if not SECURITY_MATRIX_PATH.exists():
        findings.append("legacy-docs/agents/security-specialist-matrix.md is missing")
    else:
