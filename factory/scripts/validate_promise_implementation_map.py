...
DEFAULT_MAP = REPO_ROOT / "docs" / "promise-implementation-map.public.json"
REQUIRED_CLAIM_IDS = {
...
def repo_path(ref: str) -> Path | None:
    if ref.startswith(("http://", "https://", "external:", "factoryctl:")):
        return None
    path = (ROOT / ref) if (ROOT / ref).exists() else (REPO_ROOT / ref)
...
def validate_map(path: Path = DEFAULT_MAP) -> list[str]:
    ...
    try:
        ...
        return [f"{path.relative_to(REPO_ROOT).as_posix()}: {exc}"]

    schemas = load_schemas()
    schema_ref = str(data.get("$schema") or "")
    schema = schemas.get(schema_name(schema_ref))
    if not schema:
        findings.append(f"{path.relative_to(REPO_ROOT).as_posix()}: schema not found for {schema_ref}")
    else:
        for error in validate_node(schema, data, "$", schemas=schemas, root_schema=schema):
            findings.append(f"{path.relative_to(REPO_ROOT).as_posix()}: {error}")
    ...
    for index, claim in enumerate(claims):
        ...
        if isinstance(implementation_refs, list):
            if not any(str(ref).startswith(("scripts/", "adapters/", "agents/", "schemas/", "templates/", "plugins/", "factory/scripts/", "factory/adapters/", "factory/agents/", "factory/schemas/", "factory/templates/", "factory/plugins/")) for ref in implementation_refs):
        ...
        if isinstance(proof_refs, list):
            if not any(str(ref).startswith(("tests/", "scripts/", "factory/tests/", "factory/scripts/")) for ref in proof_refs):
