...
MANIFEST_PATH = ROOT / "legacy-docs" / "public-surface.manifest.json"
...
def repo_path(root: Path, rel: str) -> Path:
    candidates = [REPO_ROOT / rel, root / rel] if REPO_ROOT != root else [root / rel]
...
def stage_map_count(root: Path) -> int:
    text = (root / "legacy-docs" / "agents" / "factory-stage-agent-map.md").read_text(encoding="utf-8")
