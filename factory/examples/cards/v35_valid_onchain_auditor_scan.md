```json
{
  "card_id": "KFP-V35-POS-ONCHAIN-AUDITOR",
  "slice_id": "OVERKILL_FACTORY_V35_10",
  "owner_profile": "onchain-worker",
  "source_refs": ["Overkill Factory v3.5 agent-workforce methodology", "solanabr/Auditor requirement", "Quasar non-Anchor constraint"],
  "source_state": "compiled",
  "outcome": "Positive gate test: Solana/Quasar R3 work can reach ready only with Auditor required and security scan packet.",
  "acceptance_criteria": ["promote passes with Quasar package, Auditor required and Codex Security scan packet"],
  "scope_in": ["gate validation only", "onchain work-package contract"],
  "scope_out": ["program edit", "deploy", "devnet", "mainnet", "signing", "funds"],
  "target_repo_paths": ["docs/factory/factory-v35-10"],
  "conflict_set": ["Example Solana program uses Quasar, not Anchor; onchain work needs Auditor path."],
  "risk_class": "R3-financial-critical",
  "why_this_class": "Onchain protocol surface is financial-critical even in gate-test form.",
  "codex_mode": "advisory",
  "why_codex_mode": "Codex observes and validates gates only.",
  "product_security_checks": ["no product approval"],
  "code_security_checks": ["Auditor and Codex Security scan packet required"],
  "system_security_checks": ["no devnet/mainnet or keys"],
  "process_security_checks": ["Auditor required, role-separated security review, human gate declared"],
  "verify_commands": ["hermes kanban promote <task_id> --json"],
  "evidence_expected": ["promotion accepted by Overkill v3.5 ready gate"],
  "review": {
    "QA_required": true,
    "independent_review_required": true,
    "security_review_required": true,
    "cybersecurity_review_required": true,
    "CTO_gate_required": true,
    "human_gate_required": true
  },
  "rollback_or_recovery": "Keep ready for test only; real done requires existing evidence, sandbox invariants, rollback and approvals.",
  "protected_assets": ["example program", "example token authority", "signer authority"],
  "security_role_separation": true,
  "human_gate_packet": {
    "gate_type": "R3_onchain_sensitive",
    "required_approvers": ["product-owner", "CTO"],
    "decision_state": "pending_before_execution",
    "risk_owner": "product-owner",
    "security_owner": "security-reviewer",
    "rollback_owner": "onchain-worker",
    "waiver_policy": "no waiver without explicit human record",
    "operator_briefing_package_ref": "reports/val-solana-quasar-r3/human-gate/operator-briefing-package.json",
    "approval_request_ref": "reports/val-solana-quasar-r3/human-gate/APPROVAL_REQUEST.json",
    "evidence_index_ref": "reports/val-solana-quasar-r3/human-gate/EVIDENCE_INDEX.json",
    "owner_review_ref": "reports/val-solana-quasar-r3/human-gate/OWNER_REVIEW.md",
    "required_decision_assets": [
      "markdown_document",
      "pdf_document",
      "approval_request_json",
      "evidence_index_json",
      "owner_review_markdown"
    ],
    "optional_explainer_assets": ["diagram", "video_explainer", "audio_explainer"],
    "decision_package_delivery": {
      "operator_interface": "telegram",
      "primary_language": "pt-BR",
      "push_required": true,
      "summary_only_forbidden": true,
      "material_before_question": true,
      "attachment_order": ["pdf_document", "video_explainer", "diagram", "markdown_document"],
      "delivery_receipt_ref": "reports/val-solana-quasar-r3/human-gate/DELIVERY_RECEIPT.json",
      "question_after_material_delivery": true
    }
  },
  "factory_method_version": "OVERKILL_V3_5_FACTORY_10",
  "phase": "F7",
  "surfaces": ["solana-quasar", "account-pda", "cpi", "compute-units"],
  "risk_initial": "R3",
  "risk_effective": "R3",
  "authority_max": "validate_gate_only",
  "owner_worker": "Solana/Quasar Architect",
  "executor_identity": "onchain-worker",
  "reviewer_identity": "security-reviewer",
  "reviewer_selection_plan": {
    "record_type": "reviewer_selection_plan",
    "changed_surfaces": ["solana-quasar", "account-pda", "cpi", "compute-units"],
    "risk_effective": "R3",
    "executor_identity": "onchain-worker",
    "forbidden_reviewers": ["onchain-worker"],
    "required_reviewers": ["security-reviewer", "solana-quasar-auditor", "independent-reviewer"],
    "reviewer_matrix": [
      {
        "reviewer_worker": "security-reviewer",
        "covers": ["security", "risk"],
        "reason": "R3 onchain work needs a security reviewer separate from execution.",
        "mandatory": true
      },
      {
        "reviewer_worker": "solana-quasar-auditor",
        "covers": ["solana-quasar", "account-pda", "cpi", "compute-units"],
        "reason": "Quasar onchain changes need Auditor-aware coverage.",
        "mandatory": true
      },
      {
        "reviewer_worker": "independent-reviewer",
        "covers": ["evidence", "gate-completeness"],
        "reason": "The final review must check evidence completeness apart from specialist findings.",
        "mandatory": true
      }
    ],
    "selection_rule": "For R3 onchain work, require security, Auditor and independent evidence review while excluding the executor.",
    "evidence_refs": ["solanabr/Auditor requirement", "Quasar non-Anchor constraint"]
  },
  "runtime_decision": "codex_security_required",
  "runtime_contract": {"mode": "read_only_gate_test"},
  "security_contract": {"security_boundary": "auditor_and_scan_required"},
  "forbidden_actions": ["code_edit", "deploy", "devnet_write", "mainnet_write", "signing", "key_access"],
  "done_definition": "Gate accepts ready because Quasar package, Auditor requirement and scan packet are present.",
  "transition_event_required": true,
  "kanban_transition_event_ref": "required_before_ready",
  "onchain_work_package": {
    "quasar_source_ref": "quasar_docs",
    "quasar_required": true,
    "framework_default": "quasar_non_anchor",
    "account_map": ["test_account"],
    "instruction_abi": ["test_instruction"],
    "pda_derivation": ["test_seed"],
    "cpi_allowlist": ["none"],
    "compute_unit_budget": "test_only",
    "auditor_required": true,
    "auditor_tool_ref": "solanabr/Auditor",
    "signer_boundary": {
      "signer_roles": ["test_signer"],
      "private_key_handling": "mock_only",
      "human_gate_required_for_signing": true,
      "mainnet_allowed_without_human_gate": false,
      "evidence_refs": ["README.md"]
    }
  },
  "security_scan_packet": {
    "security_owner": "security-reviewer",
    "scanner_agent": "security-runner",
    "scan_timing": "before_done_and_before_real_deploy",
    "scan_scope": ["Quasar program package", "PDA map", "CPI allowlist", "authority boundaries"],
    "required_tools": ["codex-security:security-scan", "solanabr/Auditor"],
    "acceptance_policy": {
      "blocking_findings_allowed": false,
      "waiver_requires_human": true,
      "evidence_required": true
    }
  }
}
```
