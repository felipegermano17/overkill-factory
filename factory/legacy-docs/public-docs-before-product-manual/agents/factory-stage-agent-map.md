...
| --- | --- | --- | --- | --- | --- |
| 1. Intake | `factory-orchestrator` | `source-ledger-worker`, `memory-steward`, `control-tower-projection-worker` | Apply the intake classifier and record request type, phase, risk surface and first reducer route. | Intake classification, source refs, initial risk. | Request cannot be classified or source is missing. |
...
| 5. Product SOT | `product-sot-planner` | `factory-orchestrator`, `human-gate-clerk`, `docs-os-worker` | Produce the product or slice source of truth candidate from confirmed understanding and source claims, then deliver an owner-readable review packet before downstream work. | SOT candidate with scope in/out, acceptance, open decisions, briefing package and factory_phase_lock. | SOT candidate is treated as approval, created before understanding confirmation, or allowed to release architecture/cleanup before owner material is delivered. |
| 6. Agentic Method Router | `factory-orchestrator` | `product-architect`, `security-orchestrator`, `skill-eval-distiller` | Resolve method weight, required artifacts, workers, gates and reviews from Method OS inputs and reducer state. | Method route and rationale. | Execution starts from habit instead of method. |
...
| 8. Product Pack & Surface Pack | `factory-orchestrator` | `product-face`, `security-orchestrator`, `agent-runtime-builder` | Resolve product/surface capability coverage from the capability-pack registry and block unsupported product types. | Capability pack contract and coverage report. | Product type lacks ready pack or activated specialist pack. |
| 9. Risk, Authority, Dependencies, Access, Budget | `factory-orchestrator` | `human-gate-clerk`, `supply-chain-gate`, `cloud-infra-security-specialist`, `crypto-key-management-specialist`, `public-safety-gate` | Materialize what can be planned, executed, tested, released and operated from active frontier, risk and authority artifacts. | Readiness blockers, access capability, budget/dependency refs and frozen downstream scope. | Material execution starts without required access, authority or phase unlock. |
| 10. Security Architecture Plan | `security-orchestrator` | `product-architect`, `appsec-owasp-specialist`, `agentic-ai-security-specialist`, `cloud-infra-security-specialist`, `crypto-key-management-specialist`, `solana-quasar-auditor` | Define trust boundaries, controls, specialist owners and required security gates after Product SOT owner review and Method Contract exist. | Security Architecture Plan, architecture packet and specialist routing rationale. | Security is postponed as a final scan or starts while upstream SOT/method is still unresolved. |
...
| 12. Product Experience Plan | `product-face` | `frontend-builder`, `docs-os-worker`, `qa-verification-worker`, `wallet-transaction-builder` | Define surface behavior, states, journeys, accessibility, project design system and proof per product surface. | Product Face packet, project `DESIGN.md` contract and proof needs. | Interface work begins without surface state coverage or design-system contract. |
...
| 18. Ready Gate | `factory-orchestrator` | `supply-chain-gate`, `security-orchestrator`, `human-gate-clerk`, `public-safety-gate` | Compute whether the line is allowed to start execution from required artifacts, access, gates and worker readiness. | Ready gate verdict and blocked reason list. | Any required artifact, access, gate or worker is missing. |
...
| 29. Release or Block | `release-ops-worker` | `human-gate-clerk`, `public-safety-gate`, `detection-monitoring-worker`, `factory-orchestrator` | Record released, release candidate, done, blocked, superseded or archived from release gate evidence. | Release decision with evidence and rollback refs. | Release decision lacks authority or monitoring evidence. |
...
| Security Architect Worker | `security-orchestrator` plus the specialist resolved from the registry |
...
| Privacy/Compliance Worker | `security-orchestrator` resolves the specialist from the registry or blocks until a pack exists |
...
not a worker executor. The operator interface channel can be Telegram, Discord, Cockpit or
