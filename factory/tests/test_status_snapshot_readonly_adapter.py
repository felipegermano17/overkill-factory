...
MODULE_PATH = ROOT / "scripts" / "status_snapshot" / "status_snapshot_readonly_adapter.py"
FIXTURES = ROOT / "fixtures" / "status-snapshot-v0"
ADAPTER_RESULT_REPORT = ROOT / "reports" / "status-snapshot-readonly-adapter-result.json"
ADAPTER_REMEDIATION_REPORT = ROOT / "reports" / "status-snapshot-readonly-adapter-remediation.json"
EXPANDED_FX_CASES = "FX01,FX02,FX03,FX04,FX05,FX06,FX07,FX08,FX09,FX10,FX11,FX12,FX13,FX14,FX15,FX16,FX17,FX18"


def load_module():
    spec = importlib.util.spec_from_file_location("status_snapshot_readonly_adapter_module", MODULE_PATH)
    assert spec is not None
    module = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    sys.modules["status_snapshot_readonly_adapter_module"] = module
...
class ReadOnlyStatusSnapshotAdapterTest(unittest.TestCase):
    ...
    def base_report(self) -> dict[str, object]:
        return {
            ...
            "task_ref": "status-snapshot-adapter-test",
            "title": "<script>alert('x')</script>",
            "phase": "F12 bounded implementation / read-only adapter",
            "risk_effective": "R2",
            "source_refs": ["STATUS-SNAPSHOT", "reports/source-report.json"],
            ...
            "changed_files": ["scripts/status_snapshot/status_snapshot_readonly_adapter.py"],
            "receipt_five": {
                "changed": "worker produced adapter code",
                "artifact_paths": ["scripts/status_snapshot/status_snapshot_readonly_adapter.py"],
                "verification_commands": ["python3 -m unittest tests.test_status_snapshot_readonly_adapter"],
    ...
    def test_report_import_escapes_untrusted_text_and_preserves_traceability_tags(self) -> None:
        ...
        self.assertEqual(snapshot["traceability"]["source"], ["STATUS-SNAPSHOT", "reports/source-report.json"])
    ...
    def test_tests_directory_repo_relative_refs_are_allowed_narrowly(self) -> None:
        allowed = adapter.classify_reference("tests/test_status_snapshot_readonly_adapter.py")

        self.assertTrue(allowed.allowed, allowed)
        self.assertEqual(allowed.kind, "relative_artifact")
        for unsafe_ref in (
            "/tmp/tests/test_status_snapshot_readonly_adapter.py",
            "../tests/test_status_snapshot_readonly_adapter.py",
            "tests/../reports/status-snapshot-readonly-adapter-result.json",
            "tests//test_status_snapshot_readonly_adapter.py",
            "file://tests/test_status_snapshot_readonly_adapter.py",
            "http://example.com/tests/test_status_snapshot_readonly_adapter.py",
            "https://169.254.169.254/tests/test_status_snapshot_readonly_adapter.py",
        ):
    ...
    def test_report_import_accepts_tests_artifact_refs_without_security_negative(self) -> None:
        ...
        report["changed_files"] = [
            "scripts/status_snapshot/status_snapshot_readonly_adapter.py",
            "tests/test_status_snapshot_readonly_adapter.py",
        ]
        ...
        self.assertEqual(snapshot["current_state"], "blocked")
        self.assertEqual(snapshot["next_safe_action"]["action_type"], "review")
        self.assertIn("tests/test_status_snapshot_readonly_adapter.py", snapshot["receipt_five_status"]["artifact_paths"])
        self.assertIn("tests/test_status_snapshot_readonly_adapter.py", snapshot["public_private_boundary"]["allowed_refs"])
        self.assertFalse(
            any("tests/test_status_snapshot_readonly_adapter.py" in ref for ref in snapshot["public_private_boundary"]["blocked_refs"]),
    ...
    def test_unsafe_non_evidence_ref_fields_are_redacted_and_fail_closed(self) -> None:
        ...
        report["source_refs"] = [private_runtime_ref, "STATUS-SNAPSHOT"]
        ...
        report["changed_files"] = [private_runtime_ref, "scripts/status_snapshot/status_snapshot_readonly_adapter.py"]
        receipt = report["receipt_five"]
        assert isinstance(receipt, dict)
        ...
        self.assertEqual(snapshot["current_state"], "security_negative")
        self.assertEqual(snapshot["next_safe_action"]["action_type"], "block")
        self.assertIn("STATUS-SNAPSHOT", snapshot["source_refs"])
    ...
    def test_expanded_fixture_inventory_command_is_spelled_out_for_public_replay(self) -> None:
        command = (
            "python3 scripts/status_snapshot/validate_status_snapshot_fixtures.py "
            "fixtures/status-snapshot-v0 "
