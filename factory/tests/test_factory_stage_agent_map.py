...
class FactoryStageAgentMapTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.stage_map = (ROOT / "legacy-docs" / "agents" / "factory-stage-agent-map.md").read_text(
            encoding="utf-8"
        )
        self.workflow_catalog = json.loads(
            (ROOT / "legacy-docs" / "factory-workflow.catalog.json").read_text(encoding="utf-8")
        )
    ...
    def test_agent_contract_language_does_not_grant_free_route_authority(self) -> None:
        checked_text = "\n".join(
            [
                ...
                (ROOT / "legacy-docs" / "agents" / "worker-profiles.md").read_text(encoding="utf-8"),
                (ROOT / "agents" / "worker-roster.md").read_text(encoding="utf-8"),
                (ROOT / "legacy-docs" / "control-tower" / "discord-control-tower-os.md").read_text(
                    encoding="utf-8"
                ),
            ]
        )
