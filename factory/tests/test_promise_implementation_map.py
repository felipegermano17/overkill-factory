...
ROOT = FACTORY_ROOT
MODULE_PATH = FACTORY_ROOT / "scripts" / "validate_promise_implementation_map.py"
...
class PromiseImplementationMapTests(unittest.TestCase):
    ...
    def test_promise_map_covers_required_public_claims(self) -> None:
        data = json.loads((ROOT / "legacy-docs" / "promise-implementation-map.public.json").read_text(encoding="utf-8"))
    ...
    def test_cli_and_ci_include_promise_map_validator(self) -> None:
        ...
        validation = (ROOT / "legacy-docs" / "operations" / "validation-and-release.md").read_text(encoding="utf-8")
        install = (REPO_ROOT / "docs" / "installation-and-use.md").read_text(encoding="utf-8")

        command = "python scripts/validate_promise_implementation_map.py"
        for text in [workflow, validation, install]:
