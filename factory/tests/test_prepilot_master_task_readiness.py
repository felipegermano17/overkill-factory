...
class PrepilotMasterTaskReadinessTest(unittest.TestCase):
    ...
    def test_production_readiness_defaults_to_generated_tmp_outputs(self) -> None:
        ...
        tmp_root = FACTORY_ROOT / ".tmp" / "factory-runs"
    ...
    def test_public_readme_rejects_narrative_history_as_onboarding(self) -> None:
        ...
        normalized = " ".join((readme + "\n" + layout).split())

        self.assertIn("Historical evidence, private runtime output, old pilot material", normalized)
        self.assertIn("should not live in the repository", normalized)
