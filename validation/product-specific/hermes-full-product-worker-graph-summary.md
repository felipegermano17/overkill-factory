...
Review status: PASS for bounded public validation; NOT_COMPLETE for practical factory completion.
...
- `python3 scripts/factory_completion_audit.py` -> exit 0, `NOT_COMPLETE`
- `python3 scripts/factory_completion_audit.py --no-write --require-complete` -> exit 1, expected because completion remains blocked
...
The staged graph is acceptable as a public, bounded, product-specific validation graph for QVG. It should not be promoted as production readiness or practical 10/10 completion until the listed production blockers have current product-specific or provider-backed evidence and the completion audit changes from `NOT_COMPLETE` through the proper gate.
