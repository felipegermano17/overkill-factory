...
DATA_BUILDER_PATH = ROOT / "scripts" / "status_snapshot" / "build_local_cockpit_data.py"
...
def load_data_builder():
    spec = importlib.util.spec_from_file_location("local_status_cockpit_data_builder", DATA_BUILDER_PATH)
    assert spec is not None
    module = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    sys.modules["local_status_cockpit_data_builder"] = module
    spec.loader.exec_module(module)
    return module


def load_factoryctl():
    spec = importlib.util.spec_from_file_location("factoryctl_for_local_status_cockpit_ui", FACTORYCTL_PATH)
    assert spec is not None
    module = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    sys.modules["factoryctl_for_local_status_cockpit_ui"] = module
    spec.loader.exec_module(module)
    return module
...
def sample_product_face_packet(tmp_path: Path) -> Path:
    payload = {
        ...
        "product_face_packet": {
            ...
            "mode": "greenfield planning packet for local status cockpit only",
...
class LocalStatusCockpitUITest(unittest.TestCase):
    def test_builder_generates_public_safe_cockpit_data_from_fixtures_and_packet_summary(self) -> None:
        ...
        self.assertEqual(data["record_type"], "local_status_cockpit_ui_data")
    ...
    def test_server_defaults_to_loopback_and_rejects_public_binding(self) -> None:
        server_path = ROOT / "scripts" / "status_snapshot" / "serve_local_cockpit.py"
        spec = importlib.util.spec_from_file_location("local_status_cockpit_server", server_path)
        assert spec is not None
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        sys.modules["local_status_cockpit_server"] = module
        spec.loader.exec_module(module)
