...
| 9. Risk, Authority, Dependencies, Access, Budget | `factory-orchestrator` | `human-gate-clerk`, `supply-chain-gate`, `cloud-infra-security-specialist`, `crypto-key-management-specialist`, `public-safety-gate` | Decide what can be planned, executed, tested, released and operated only when the active frontier allows it. | Readiness blockers, access capability, budget/dependency refs and frozen downstream scope. | Material execution starts without required access, authority or phase unlock. |
