...
| `human-gate-clerk` | Prepares and records real human decisions for authority, access, budget, waiver and release scope. | F9, F15, F16. | gate packet, operator briefing package, approval request, evidence index, owner review, markdown/PDF decision assets, evidence, rollback, access capability, budget contract, waiver request. | `human_gate_record`. | Invent approval, ask for a decision before delivering material, or merge access, budget and release into one vague decision. | decision, approval scope, actor role, durable runtime event, evidence refs. | Hermes decision record. |
