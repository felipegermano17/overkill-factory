...
| `independent-reviewer` | Reviews evidence produced by a different executor. | F15. | executor, reviewer, done definition, reviewer selection plan. | `independent_review_result`. | Review its own work or replace human gates. | review report, selected coverage, findings, residual risk. | independent Hermes/Codex profile. |
...
| `human-gate-clerk` | Prepares and records real human decisions for authority, access, budget, waiver and release scope. | F9, F15, F16. | phase lock, gate packet, operator briefing package, approval request, evidence index, owner review, markdown/PDF decision assets, evidence, rollback, access capability, budget contract, waiver request. | `human_gate_record`. | Invent approval, ask for a decision before delivering material, ask for frozen downstream work, or merge access, budget and release into one vague decision. | decision, approval scope, actor role, durable runtime event, evidence refs, delivered package refs. | Hermes decision record. |
