...
DOWNSTREAM_PHASE_LOCK_SURFACES = (
    ARCHITECTURE_SURFACES
    | REPO_CLEANUP_SURFACES
    | SUPPLY_CHAIN_SURFACES
    | SPECIFIC_BUILDER_SURFACES
...
def factory_phase_rank(phase_id: Any) -> int:
    value = str(phase_id or "").strip().upper()
    match = re.fullmatch(r"F(\d+)([A-Z]?)", value)
    if not match:
        return -1
    base = int(match.group(1)) * 10
    suffix = match.group(2)
    if suffix:
        base += ord(suffix) - ord("A") + 1
    return base
...
def _phase_lock_ref(value: Any) -> str:
    return str(value or "").strip()


def _phase_lock_public_ref_ok(value: Any) -> bool:
    ref = _phase_lock_ref(value)
    if not ref:
        return False
    _, redaction = sanitize_public_ref(ref)
    return redaction is None


def _factory_phase_lock(card: dict[str, Any]) -> dict[str, Any]:
    lock = card.get("factory_phase_lock")
...
def factory_phase_lock_projection(card: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "card_phase_id": str(card.get("phase") or "").strip(),
        "single_active_frontier": lock.get("single_active_frontier") is True if lock else True,
        "owner_surface_first": sot_delivered,
        "product_sot_review_packet_delivered": sot_delivered,
        "method_contract_materialized": method_done,
        "ready_gate_materialized": ready_done,
        "downstream_freeze_active": downstream_freeze,
        "downstream_surface_seen": downstream_surface_seen,
        "frozen_phase_ids": _list_items(lock.get("frozen_phase_ids")),
        "frozen_worker_ids": _list_items(lock.get("frozen_worker_ids")),
        "next_required_artifact": str(
            lock.get("next_required_artifact")
            or FACTORY_PHASE_LOCK_NEXT_ARTIFACTS.get(frontier)
            or "operator_briefing_package"
        ),
        "operator_decision_required": lock.get("operator_decision_required") is True,
    }


def factory_phase_lock_errors(card: dict[str, Any]) -> list[str]:
    if card.get("factory_method_version") != "OVERKILL_VFINAL":
        return []
    ...
    if lock:
        schemas = bundled_schemas()
        schema = schemas.get("factory-phase-lock.schema.json")
        if schema:
            errors.extend(validate_node(schema, lock, "factory_phase_lock", schemas=schemas, root_schema=schema))
        frontier = str(lock.get("active_frontier") or "").strip()
        if frontier and frontier not in FACTORY_PHASE_LOCK_FRONTIERS:
            errors.append(f"factory_phase_lock.active_frontier unknown frontier {frontier!r}")
        current_phase_id = _factory_phase_lock_current_phase_id(card)
        if current_phase_id and factory_phase_rank(current_phase_id) < 0:
            errors.append(f"factory_phase_lock.current_phase_id unknown phase {current_phase_id!r}")
        for field, value in lock.items():
            if field.endswith("_ref") and str(value or "").strip() and not _phase_lock_public_ref_ok(value):
    ...
    surfaces = normalized_surfaces(card)
    downstream_surface_seen = bool(surfaces & DOWNSTREAM_PHASE_LOCK_SURFACES)
    downstream_phase_seen = phase_rank >= factory_phase_rank("F8")
    allowed_surfaces = _phase_lock_allowed_surfaces(str(projection["active_frontier"]))
    future_surface_seen = bool((surfaces - allowed_surfaces) & DOWNSTREAM_PHASE_LOCK_SURFACES)
    ...
    if current_rank >= 0 and phase_rank > current_rank and (projection["downstream_freeze_active"] or future_surface_seen):
        errors.append(
            "factory_phase_lock blocks downstream card phase "
            f"{card.get('phase')} while active frontier is {projection['active_frontier']}"
        )

    if (downstream_phase_seen or downstream_surface_seen) and not projection["product_sot_review_packet_delivered"]:
        errors.append(
            "factory_phase_lock owner-surface-first requires an owner-readable Product SOT review packet "
            "before architecture, repo cleanup, human gates, worker packets or execution"
        )

    if (downstream_phase_seen or downstream_surface_seen) and not projection["method_contract_materialized"]:
        errors.append(
            "factory_phase_lock requires Method Contract before architecture, repo cleanup, human gates, worker packets or execution"
        )

    if surfaces & REPO_CLEANUP_SURFACES and not projection["ready_gate_materialized"]:
        errors.append(
...
def validate_vfinal_card_contract(data: dict[str, Any]) -> list[str]:
    ...
    errors.extend(factory_phase_lock_errors(data))
...
def _source_ledger_next_artifact(source_resolution: dict[str, Any]) -> dict[str, Any]:
    artifacts = source_resolution.get("next_artifacts") if isinstance(source_resolution.get("next_artifacts"), list) else []
    seen_source_ledger = False
    for artifact in artifacts:
        if not isinstance(artifact, dict):
            continue
        artifact_type = str(artifact.get("artifact_type") or "").strip()
        if artifact_type == "source_ledger":
            seen_source_ledger = True
            continue
        if seen_source_ledger and artifact_type:
            return artifact
    return {
        "artifact_type": "source_resolution",
        "owner_worker": "source-ledger-worker",
    }


def build_product_source_ledger(
    source_resolution: dict[str, Any],
    *,
    source_ref_public_safe: str,
    created_at: str | None = None,
    ledger_id: str | None = None,
) -> dict[str, Any]:
    source_resolution_errors = validate_source_resolution_packet(source_resolution)
    if source_resolution_errors:
        raise ValueError("; ".join(source_resolution_errors))

    _, redaction = sanitize_public_ref(source_ref_public_safe)
    if redaction is not None:
        raise ValueError("source_ref_public_safe must be public-safe")

    source_signal = copy.deepcopy(source_resolution.get("source_signal") or {})
    source_resolution_ref = str(source_resolution.get("packet_id") or "source-resolution-packet")
    created = created_at or datetime.now(timezone.utc).replace(microsecond=0).isoformat()
    next_artifact = _source_ledger_next_artifact(source_resolution)
    next_artifact_type = str(next_artifact.get("artifact_type") or "source_resolution")
    next_worker = str(next_artifact.get("owner_worker") or "source-ledger-worker")
    needs_product_sot = source_signal.get("needs_product_sot") is True

    return {
        "$schema": "https://overkill-factory.dev/schemas/product-source-ledger.schema.json",
        "record_type": "product_source_ledger",
        "ledger_id": ledger_id or f"source-ledger-{slug_for_ref(str(source_signal.get('intake_id') or source_resolution_ref))}",
        "created_at": created,
        "factory_method_version": "OVERKILL_VFINAL",
        "source_resolution_ref": source_resolution_ref,
        "source_ref_public_safe": source_ref_public_safe,
        "source_signal": source_signal,
        "source_map": [
            {
                "source_id": "source-001",
                "source_ref_public_safe": source_ref_public_safe,
                "source_class": str(source_signal.get("source_class") or "operator_supplied"),
                "sensitivity_class": str(source_signal.get("sensitivity_class") or "public_safe"),
                "freshness": str(source_signal.get("freshness") or "unknown"),
                "raw_private_embedded": False,
            }
        ],
        "claim_table": [
            {
                "claim_id": "claim-001",
                "claim_class": "fact",
                "claim_text_public_safe": str(source_signal.get("summary_public_safe") or "A source signal exists."),
                "source_refs": [source_ref_public_safe],
                "status": "recorded",
                "blocks_product_sot_when_unresolved": False,
            },
            {
                "claim_id": "gap-001",
                "claim_class": "gap",
                "claim_text_public_safe": (
                    "Product SOT still requires worker-owned claim review and outcome framing."
                    if needs_product_sot
                    else "The next route artifact still requires worker-owned claim review."
                ),
                "source_refs": [source_resolution_ref],
                "status": "gap",
                "blocks_product_sot_when_unresolved": needs_product_sot,
            },
        ],
        "conflicts": [],
        "gaps": [
            {
                "gap_id": "gap-001",
                "description_public_safe": (
                    "Product SOT has not been generated by the factory worker yet."
                    if needs_product_sot
                    else f"{next_artifact_type} has not been generated by the factory worker yet."
                ),
                "owner_worker": next_worker,
                "blocks_product_sot": needs_product_sot,
            }
        ],
        "resolution_state": {
            "source_ledger_created": True,
            "claim_resolution_complete": False,
            "no_chat_only_state": True,
        },
        "blocking_rules": {
            "product_sot_blocked_until_claims_reviewed": needs_product_sot,
            "execution_blocked_until_required_artifacts_pass": True,
        ...
        "acceptance": {
            ...
            "execution_allowed": False,
            "blocked_reason": (
                "Source ledger exists, but execution remains blocked until outcome, Product SOT when required, "
...
def validate_product_source_ledger(ledger: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    schemas = bundled_schemas()
    schema = schemas.get("product-source-ledger.schema.json")
    if not schema:
        return ["product_source_ledger schema is not bundled"]
    errors.extend(
        validate_node(
            schema,
            ledger,
    ...
    for field in ("source_resolution_ref", "source_ref_public_safe"):
        value = str(ledger.get(field) or "").strip()
        if value:
            _validate_public_ref(value, f"product_source_ledger.{field}", errors)

    source_signal = ledger.get("source_signal") if isinstance(ledger.get("source_signal"), dict) else {}
    signal_ref = str(source_signal.get("signal_ref_public_safe") or "").strip()
    if signal_ref:
        _validate_public_ref(signal_ref, "product_source_ledger.source_signal.signal_ref_public_safe", errors)

    source_map = ledger.get("source_map") if isinstance(ledger.get("source_map"), list) else []
    for index, source in enumerate(source_map):
        if not isinstance(source, dict):
            continue
        source_ref = str(source.get("source_ref_public_safe") or "").strip()
        if source_ref:
            _validate_public_ref(source_ref, f"product_source_ledger.source_map[{index}].source_ref_public_safe", errors)
        if source.get("raw_private_embedded") is not False:
            errors.append(f"product_source_ledger.source_map[{index}].raw_private_embedded must be false")

    claim_table = ledger.get("claim_table") if isinstance(ledger.get("claim_table"), list) else []
    claim_classes = {
    ...
    if "fact" not in claim_classes:
        errors.append("product_source_ledger requires at least one fact claim")
    for index, claim in enumerate(claim_table):
        if not isinstance(claim, dict):
            continue
        refs = claim.get("source_refs") if isinstance(claim.get("source_refs"), list) else []
        for ref_index, ref in enumerate(refs):
            _validate_public_ref(str(ref), f"product_source_ledger.claim_table[{index}].source_refs[{ref_index}]", errors)
        if claim.get("claim_class") == "conflict" and claim.get("status") == "promoted":
            errors.append(f"product_source_ledger.claim_table[{index}] cannot promote unresolved conflict claims")

    resolution_state = ledger.get("resolution_state") if isinstance(ledger.get("resolution_state"), dict) else {}
    if resolution_state.get("source_ledger_created") is not True:
        errors.append("product_source_ledger.resolution_state.source_ledger_created must be true")
    if resolution_state.get("no_chat_only_state") is not True:
        errors.append("product_source_ledger.resolution_state.no_chat_only_state must be true")

    blocking_rules = ledger.get("blocking_rules") if isinstance(ledger.get("blocking_rules"), dict) else {}
    for field in (
        "execution_blocked_until_required_artifacts_pass",
        ...
        if blocking_rules.get(field) is not True:
            errors.append(f"product_source_ledger.blocking_rules.{field} must be true")
    if not isinstance(blocking_rules.get("product_sot_blocked_until_claims_reviewed"), bool):
        errors.append("product_source_ledger.blocking_rules.product_sot_blocked_until_claims_reviewed must be boolean")

    handoff = ledger.get("handoff") if isinstance(ledger.get("handoff"), dict) else {}
    if handoff.get("factory_owned_next_step") is not True:
        errors.append("product_source_ledger.handoff.factory_owned_next_step must be true")
    next_worker = str(handoff.get("next_worker") or "").strip()
    if next_worker and next_worker not in WORKERS:
        errors.append(f"product_source_ledger.handoff.next_worker must be a registered worker: {next_worker}")

    boundary = ledger.get("public_private_boundary") if isinstance(ledger.get("public_private_boundary"), dict) else {}
    if boundary.get("public_safe_refs_only") is not True:
        errors.append("product_source_ledger requires public_safe_refs_only=true")
    if boundary.get("raw_private_evidence_embedded") is not False:
        errors.append("product_source_ledger must not embed raw private evidence")
    if boundary.get("private_context_retained_outside_public_repo") is not True:
        errors.append("product_source_ledger private context must stay outside the public repo")

    acceptance = ledger.get("acceptance") if isinstance(ledger.get("acceptance"), dict) else {}
    if acceptance.get("source_ledger_created") is not True:
        errors.append("product_source_ledger acceptance.source_ledger_created must be true")
    if acceptance.get("product_sot_generated") is not False:
        errors.append("product_source_ledger must not claim Product SOT was generated")
    if acceptance.get("execution_allowed") is not False:
        errors.append("product_source_ledger acceptance.execution_allowed must be false")
    _validate_lifecycle_refs(
        _list_items(acceptance.get("evidence_refs")),
...
def _product_creation_plan_proof_ids(plan: dict[str, Any]) -> list[str]:
...
def build_product_implementation_readiness(
    ...
    *,
    ...
    ready_units = [
        str(unit.get("unit_id") or "").strip()
        for unit in work_units
        if str(unit.get("status") or "").strip() not in {"blocked", "rejected", "superseded", "needs_refresh"}
    ]
    ready_units = [unit_id for unit_id in ready_units if unit_id]
    blocked_units = [unit_id for unit_id in blocked_units if unit_id]
    human_decisions = _dedupe_preserve_order(
        [
            str(unit.get("blocker_id") or "")
            for unit in work_units
            if str(unit.get("status") or "").strip() == "blocked"
            and (
                str(unit.get("blocker_owner") or "").strip() == "human-gate-clerk"
                or str(unit.get("blocker_id") or "").startswith("human-gate")
            )
        ]
    )
    ...
    allowed_execution_scope = "ready_work_units_only" if material_execution_allowed else "none"

    concern_items = []
    if alignment_result == "CONCERNS":
        concern_items.append(
            {
                "concern_id": "bounded-ready-work-units-not-product-completion",
                "severity": "MEDIUM",
                "owner": "factory-orchestrator",
                "impact": (
                    "Ready work units may execute, but they cannot claim complete-product acceptance "
                    "until every active Product SOT requirement is reconciled."
                ),
                "allowed_actions": ["execute explicitly listed ready work units without changing Product SOT scope"],
                "forbidden_actions": [
                    "claim complete product done from a bounded slice",
                    "execute work units not listed in ready_work_units",
                ],
                "allowed_ready_work_units": ready_units,
                "expiry": "until Product SOT, Method Contract, Product Creation Plan or work-unit state changes",
                "next_action": "materialize ready work units through Hermes worker packets with Receipt Five evidence.",
                "evidence_refs": [plan_ref],
            }
        )

    delivery_profile_proof_coverage = [
        {
            "proof_id": proof_id,
            "status": "PASS",
            "evidence_refs": [plan_ref],
            ...
            "basis": "Product Creation Plan requires this proof before the work unit can close.",
        }
    ...
    for unit in work_units:
        ...
        if not unit_id:
            continue
        status = "blocked" if unit_id in blocked_units else "mapped"
        for requirement_ref in _list_items(unit.get("product_sot_requirement_refs")):
            requirements_trace.append(
                {
                    "requirement_ref": requirement_ref,
                    "work_unit_refs": [unit_id],
                    "proof_refs": _list_items(unit.get("proof_ids_required")),
                    "status": status,
                }
            )

    allowed_next_actions = (
        ["resolve blocked work units through their named owners before material execution"]
        if alignment_result == "BLOCKED"
        else ["execute ready_work_units only, preserving canonical Product SOT scope"]
    )
    forbidden_next_actions = [
        "claim complete product done from one bounded slice",
        "execute blocked or unlisted work units",
        "release, deploy, mainnet, customer-ready or irreversible changes before promotion gates",
    ]

    readiness: dict[str, Any] = {
        "$schema": "https://overkill-factory.dev/schemas/product-implementation-readiness.schema.json",
        ...
        "product_delivery_quality_profile_trace": _dedupe_preserve_order(
            [
                str(product_creation_plan.get("product_delivery_quality_profile_ref") or ""),
                *[f"{proof_id} is required before implementation" for proof_id in proof_ids],
            ]
        ),
        "delivery_profile_proof_coverage": delivery_profile_proof_coverage,
        "artifact_alignment_result": alignment_result,
        ...
        "capability_pack_trace": _list_items(product_creation_plan.get("capability_pack_requirements")),
        "access_and_secret_trace": [
            (
                f"{access_requirement_count} access/secret requirement(s) must be resolved by "
                "the readiness gate or worker packets; no secret values are embedded."
            )
        ],
        "proof_coverage": proof_ids,
        "stale_artifacts": [],
        "contradictions": [],
        "ready_work_units": [] if alignment_result == "BLOCKED" else ready_units,
        ...
        "handoff": {
            "next_artifact": "worker_packets",
            "next_worker": "factory-orchestrator",
            "next_safe_action": (
                "Resolve readiness blockers before material execution."
                if alignment_result == "BLOCKED"
                else "Materialize only ready_work_units as Hermes worker packets with Receipt Five evidence."
            ),
            "user_decision_required": bool(human_decisions),
            "factory_owned_next_step": True,
        },
        "public_private_boundary": {
            "public_safe_refs_only": True,
        ...
        "acceptance": {
            "product_implementation_readiness_created": True,
...
def validate_product_implementation_readiness(readiness: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    schemas = bundled_schemas()
    schema = schemas.get("product-implementation-readiness.schema.json")
    if not schema:
        return ["product_implementation_readiness schema is not bundled"]
    errors.extend(validate_node(schema, readiness, "product_implementation_readiness", schemas=schemas, root_schema=schema))
    if readiness.get("record_type") != "product_implementation_readiness":
        errors.append("product_implementation_readiness.record_type must be product_implementation_readiness")
    result = str(readiness.get("artifact_alignment_result") or "").strip().upper()
    ...
    for field in (
        ...
        if not (readiness.get(field) if isinstance(readiness.get(field), list) else []):
            errors.append(f"product_implementation_readiness.{field} must be non-empty")

    ready_units = _list_items(readiness.get("ready_work_units"))
    blocked_units = _list_items(readiness.get("blocked_work_units"))
    overlapping_units = sorted(set(ready_units).intersection(blocked_units))
    if overlapping_units:
        errors.append("product_implementation_readiness ready and blocked work units overlap: " + ", ".join(overlapping_units))
    if result == "PASS" and not ready_units:
        errors.append("product_implementation_readiness PASS requires ready_work_units")
    if result in {"FAIL", "BLOCKED"} and ready_units:
        errors.append("product_implementation_readiness FAIL/BLOCKED must not expose ready_work_units")
    if result == "BLOCKED" and not blocked_units:
        errors.append("product_implementation_readiness BLOCKED requires blocked_work_units")
    errors.extend(validate_product_implementation_concerns(readiness))

    for index, item in enumerate(readiness.get("requirements_trace", []) if isinstance(readiness.get("requirements_trace"), list) else []):
        if not isinstance(item, dict):
            continue
        for field in ("requirement_ref", "work_unit_refs", "proof_refs", "status"):
            if not item.get(field):
                errors.append(f"product_implementation_readiness.requirements_trace[{index}].{field} is required")
        for ref_index, ref in enumerate(_list_items(item.get("work_unit_refs"))):
            _validate_public_ref(ref, f"product_implementation_readiness.requirements_trace[{index}].work_unit_refs[{ref_index}]", errors)
    ...
    for field in (
        ...
        if blocking_rules.get(field) is not True:
            errors.append(f"product_implementation_readiness.blocking_rules.{field} must be true")

    handoff = readiness.get("handoff") if isinstance(readiness.get("handoff"), dict) else {}
    if handoff.get("factory_owned_next_step") is not True:
        errors.append("product_implementation_readiness.handoff.factory_owned_next_step must be true")
    next_worker = str(handoff.get("next_worker") or "").strip()
    if next_worker and next_worker not in WORKERS:
        errors.append(f"product_implementation_readiness.handoff.next_worker must be a registered worker: {next_worker}")

    boundary = readiness.get("public_private_boundary") if isinstance(readiness.get("public_private_boundary"), dict) else {}
    if boundary.get("public_safe_refs_only") is not True:
        errors.append("product_implementation_readiness requires public_safe_refs_only=true")
    if boundary.get("raw_private_evidence_embedded") is not False:
        errors.append("product_implementation_readiness must not embed raw private evidence")
    if boundary.get("private_context_retained_outside_public_repo") is not True:
        errors.append("product_implementation_readiness private context must stay outside the public repo")

    acceptance = readiness.get("acceptance") if isinstance(readiness.get("acceptance"), dict) else {}
    if acceptance.get("product_implementation_readiness_created") is not True:
        errors.append("product_implementation_readiness acceptance.product_implementation_readiness_created must be true")
    ...
    if result in {"FAIL", "BLOCKED"}:
        if material_allowed is not False:
            errors.append("product_implementation_readiness FAIL/BLOCKED requires material_execution_allowed=false")
        if scope != "none":
            errors.append("product_implementation_readiness FAIL/BLOCKED requires allowed_execution_scope=none")
    if result == "CONCERNS":
        if material_allowed is not bool(ready_units):
            errors.append("product_implementation_readiness CONCERNS material_execution_allowed must match ready_work_units")
        if ready_units and scope != "ready_work_units_only":
            errors.append("product_implementation_readiness CONCERNS with ready units requires allowed_execution_scope=ready_work_units_only")
    if result == "PASS" and material_allowed is not True:
        errors.append("product_implementation_readiness PASS requires material_execution_allowed=true")
    if acceptance.get("complete_product_claim_allowed") is not False:
        errors.append("product_implementation_readiness complete_product_claim_allowed must be false")
    _validate_lifecycle_refs(
        _list_items(acceptance.get("evidence_refs")),
        "product_implementation_readiness.acceptance.evidence_refs",
        errors,
    )
...
def command_validate_source_ledger(args: argparse.Namespace) -> int:
    errors = validate_product_source_ledger(load_json_like(args.path))
    if errors:
        for error in errors:
            print(error, file=sys.stderr)
        return 1
...
def command_validate_product_implementation_readiness(args: argparse.Namespace) -> int:
    errors = validate_product_implementation_readiness(load_json_like(args.path))
    if errors:
        for error in errors:
            print(error, file=sys.stderr)
        return 1
...
def command_source_ledger(args: argparse.Namespace) -> int:
    try:
        ledger = build_product_source_ledger(
            load_json_like(args.source_resolution),
            source_ref_public_safe=args.source_ref,
            created_at=args.created_at,
        ...
        print(str(exc), file=sys.stderr)
        return 1
    errors = validate_product_source_ledger(ledger)
    if errors:
        for error in errors:
            print(error, file=sys.stderr)
        return 1
...
def command_product_implementation_readiness(args: argparse.Namespace) -> int:
    ...
    args.out.write_text(json.dumps(readiness, indent=2, ensure_ascii=True) + "\n", encoding="utf-8")
    print(f"Wrote {public_path_ref(args.out)}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    validate_source_ledger_parser = sub.add_parser("validate-source-ledger")
    ...
    source_ledger_parser = sub.add_parser(
        "source-ledger",
        help="Build a product source ledger from a validated source-resolution packet without generating Product SOT.",
    )
    source_ledger_parser.add_argument("--source-resolution", type=Path, required=True)
    source_ledger_parser.add_argument("--source-ref", required=True)
    source_ledger_parser.add_argument("--created-at")
