...
def _is_source_label(text: str) -> bool:
    return bool(
        text.startswith("SRC-")
        or text in {"STATUS-SNAPSHOT", "METHOD-CONTRACT", "DOCS-OS", "HUMAN-GATE", "PRODUCT-SOT"}
        ...
        evidence_refs.append(
            {
                "id": "EV-ADAPTER-INPUT",
                "kind": "adapter_input",
                "ref": "reports/status-snapshot-readonly-adapter-input.json",
...
def _base_snapshot(*, fixture_id: str, fixture_name: str, title: str, source_refs: list[str]) -> dict[str, Any]:
    return {
        ...
        "source_refs": source_refs or ["STATUS-SNAPSHOT"],
        ...
        "traceability": {"source": source_refs or ["STATUS-SNAPSHOT"], "inference": [], "decision": []},
    }
...
def _missing_snapshot(path: Path) -> dict[str, Any]:
    ...
    snapshot["receipt_five_status"] = {
        ...
        "artifact_paths": ["reports/status-snapshot-readonly-adapter-missing-input.json"],
        "verification_commands": ["python3 scripts/status_snapshot/status_snapshot_readonly_adapter.py import <input>"],
...
def _import_status_snapshot(data: dict[str, Any], *, source_path: Path, resolver: Resolver | None = None) -> dict[str, Any]:
    ...
    snapshot.setdefault("traceability", {"source": snapshot.get("source_refs") or ["STATUS-SNAPSHOT"], "inference": [], "decision": []})
    return snapshot


def _report_snapshot(data: dict[str, Any], *, source_path: Path, resolver: Resolver | None = None) -> dict[str, Any]:
    fallback_source = repo_relative(source_path) or "STATUS-SNAPSHOT"
    source_refs, source_allowed_refs, source_blocked_refs = _normalize_ref_list(
    ...
    if not artifact_refs:
        artifact_refs = changed_file_refs or ["reports/status-snapshot-readonly-adapter-input.json"]
    ...
    verification_commands = _list_of_strings(receipt.get("verification_commands")) or ["python3 -m unittest tests.test_status_snapshot_readonly_adapter"]
    snapshot["receipt_five_status"] = {
