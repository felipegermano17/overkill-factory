#!/usr/bin/env python3
"""Build public-safe local cockpit data for the local status cockpit.
...
FIXTURE_DIR = ROOT / "fixtures" / "status-snapshot-v0"
DEFAULT_OUTPUT = ROOT / "ui" / "local-status-cockpit" / "data" / "status-cockpit.json"
...
def load_adapter_module():
    spec = importlib.util.spec_from_file_location("status_snapshot_readonly_adapter_for_cockpit", ADAPTER_MODULE_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"unable to load adapter module at {ADAPTER_MODULE_PATH}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["status_snapshot_readonly_adapter_for_cockpit"] = module
...
def clean_ref(value: Any) -> str:
    ...
    if text.startswith("https://"):
        return text
    if text.startswith("SRC-") or text in {"STATUS-SNAPSHOT", "METHOD-CONTRACT", "DOCS-OS", "HUMAN-GATE", "PRODUCT-SOT"}:
        return text
...
def report_input_paths(root: Path) -> list[Path]:
    ...
    if not reports.exists():
        return []
    patterns = [
        "status-snapshot-v0-*.json",
        "status-snapshot-readonly-adapter*.json",
    ]
    paths: list[Path] = []
...
def build_cockpit_data(
    ...
    *,
    ...
    payload = {
        "$schema": "https://overkill-factory.dev/schemas/local-status-cockpit-ui-data.schema.json",
        "record_type": "local_status_cockpit_ui_data",
        "created_at": utc_now(),
        "source_inputs": {
            "normal_fetch_scope": "local-relative-json-only",
            "status_snapshot_fixture_dir": "fixtures/status-snapshot-v0",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build local status cockpit UI data")
