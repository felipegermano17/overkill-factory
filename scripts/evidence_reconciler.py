...
def result_entry(card: dict[str, Any], path: Path, data: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "factory_sdlc_lifecycle_refs": factoryctl.string_list(data.get("factory_sdlc_lifecycle_refs")),
...
def reconcile(
    ...
    closure_fields = {
        field: {
            ...
            "sdlc_feedback_loop_ref": entry.get("sdlc_feedback_loop_ref"),
            "factory_sdlc_lifecycle_refs": entry.get("factory_sdlc_lifecycle_refs", []),
    ...
    sdlc_feedback_loop_refs = sorted(
        {
            str(ref).strip()
            for entry in effective_results.values()
            for ref in [
                entry.get("sdlc_feedback_loop_ref"),
                *entry.get("factory_sdlc_lifecycle_refs", []),
            ]
            if str(ref or "").strip()
        }
    )
    ...
    return {
        ...
        "sdlc_feedback_loop_refs": sdlc_feedback_loop_refs,
