#!/usr/bin/env python3
"""Prove dashboard/API reassign-route guard for the Hermes vFinal adapter.

This smoke runs only against a disposable installed Hermes checkout/home with
patch 0012 applied. It calls dashboard plugin `POST /tasks/{task_id}/reassign`
and verifies that Overkill Factory cards and worker subtasks cannot be manually
reassigned from dashboard/API.
...
DEFAULT_OUT = ROOT / "validation" / "hermes-installed-runtime-smoke" / "dashboard-reassign-route-guard-smoke.json"
...
def _runtime_env(checkout: Path, hermes_home: Path, board: str) -> None:
    ...
    os.environ["OVERKILL_FACTORY_RUNTIME_DIR"] = str(hermes_home / "overkill-factory-dashboard-reassign")
...
def _reexec_with_checkout_python(checkout: Path) -> None:
    if os.environ.get("OVERKILL_DASHBOARD_REASSIGN_REEXEC") == "1":
        return
    ...
    try:
        ...
        pass
    env = os.environ.copy()
    env["OVERKILL_DASHBOARD_REASSIGN_REEXEC"] = "1"
    if os.name == "nt":
...
def _task_assignee(kanban_db: Any, *, board: str, task_id: str) -> str | None:
    conn = kanban_db.connect(board=board)
    try:
        task = kanban_db.get_task(conn, task_id)
        return None if task is None else task.assignee
    finally:
        conn.close()
...
def _reassign_attempt(plugin_api: Any, *, board: str, task_id: str, profile: str) -> dict[str, Any]:
    try:
        result = plugin_api.reassign_task_endpoint(
            task_id,
            plugin_api.ReassignBody(profile=profile),
            board=board,
        )
...
def run_smoke(*, checkout: Path, hermes_home: Path, allow_non_disposable: bool) -> dict[str, Any]:
    ...
    parent_assignee_before = _task_assignee(kanban_db, board=board, task_id=parent_id)
    worker_assignee_before = _task_assignee(kanban_db, board=board, task_id=worker_id)

    normal_id = _create_task(
        plugin_api,
        board=board,
        title="NORMAL-REASSIGN-CONTROL",
        body="ordinary non-factory disposable task",
        assignee="before-profile",
    )

    parent_reassign = _reassign_attempt(plugin_api, board=board, task_id=parent_id, profile="after-profile")
    worker_reassign = _reassign_attempt(plugin_api, board=board, task_id=worker_id, profile="after-profile")
    normal_reassign = _reassign_attempt(plugin_api, board=board, task_id=normal_id, profile="after-profile")

    parent_assignee_after = _task_assignee(kanban_db, board=board, task_id=parent_id)
    worker_assignee_after = _task_assignee(kanban_db, board=board, task_id=worker_id)
    normal_assignee_after = _task_assignee(kanban_db, board=board, task_id=normal_id)
    ...
    checks = {
        ...
        "dashboard_reassign_parent_vfinal_blocked": (
            "PASS"
            if parent_reassign["ok"] is False
            and parent_reassign["http_status"] == 409
            and parent_assignee_after == parent_assignee_before
            else "FAIL"
        ),
        "dashboard_reassign_worker_subtask_blocked": (
            "PASS"
            if worker_reassign["ok"] is False
            and worker_reassign["http_status"] == 409
            and worker_assignee_after == worker_assignee_before
            else "FAIL"
        ),
        "dashboard_reassign_normal_task_allowed": (
            "PASS" if normal_reassign["ok"] is True and normal_assignee_after == "after-profile" else "FAIL"
        ),
    ...
    return {
        ...
        "card_ref": {
            "card_id": "VFINAL-HERMES-DASHBOARD-REASSIGN-ROUTE-GUARD-SMOKE",
            "slice_id": "VFINAL_HERMES_DASHBOARD_REASSIGN_ROUTE_GUARD",
            "phase": "F13",
            "risk_effective": "R3",
        ...
        "findings_summary": (
            "Dashboard/API reassign-route guard smoke passed: vFinal parent cards "
            "and worker subtasks cannot be manually reassigned, while ordinary "
            "disposable task reassign still works."
            if result == "PASS"
            else "Dashboard/API reassign-route guard smoke failed; inspect checks."
        ),
        "tool_or_profile": "Hermes v0.16.0 disposable installed runtime with vFinal adapter patches including candidate 0012",
        ...
        "patches": ["adapters/hermes/patches/0012-guard-overkill-vfinal-dashboard-reassign-route.patch"],
        "checks": checks,
        "dashboard_reassign_evidence": {
            "board_slug": board,
            ...
            "attempts": {
                "parent": parent_reassign,
                "worker": worker_reassign,
                "normal": normal_reassign,
            },
            "assignee_after": {
                "parent": parent_assignee_after,
                "worker": worker_assignee_after,
                "normal": normal_assignee_after,
            },
        },
        "important_limitations": [
            "This proves direct dashboard plugin route functions in a disposable installed Hermes runtime after candidate patch 0012.",
            "It does not start an authenticated HTTP dashboard server.",
            "It does not define a future operator-approved reassign workflow.",
        ...
        "evidence_refs": [
            ...
            "adapters/hermes/dashboard_reassign_route_guard_smoke.py",
            "validation/cards/vfinal-r3-ready.md",
            "validation/hermes-installed-runtime-smoke/dashboard-reassign-route-guard-smoke.json",
        ],
        "next_action": "Add patch 0012 to the update sequence and continue proving remaining dashboard/API route families.",
    }
...
def main() -> int:
    ...
    try:
        ...
        result = {
            ...
            "tool_or_profile": "adapters/hermes/dashboard_reassign_route_guard_smoke.py",
            "executed_by": "remote-proof-runner",
            "important_limitations": [
