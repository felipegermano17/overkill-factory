#!/usr/bin/env python3
"""Prove dashboard/API specify-route guard for Hermes vFinal.

This smoke runs only against a disposable installed Hermes checkout/home with
patch 0014 applied. It calls dashboard plugin `POST /tasks/{task_id}/specify`
and verifies that Overkill Factory cards and worker subtasks cannot be
rewritten or promoted by the generic dashboard specifier route.
...
DEFAULT_OUT = ROOT / "validation" / "hermes-installed-runtime-smoke" / "dashboard-specify-route-guard-smoke.json"
...
def _runtime_env(checkout: Path, hermes_home: Path, board: str) -> None:
    ...
    os.environ["OVERKILL_FACTORY_RUNTIME_DIR"] = str(hermes_home / "overkill-factory-dashboard-specify")
...
def _reexec_with_checkout_python(checkout: Path) -> None:
    if os.environ.get("OVERKILL_DASHBOARD_SPECIFY_REEXEC") == "1":
        return
    ...
    try:
        ...
        pass
    env = os.environ.copy()
    env["OVERKILL_DASHBOARD_SPECIFY_REEXEC"] = "1"
    if os.name == "nt":
...
def _specify_attempt(plugin_api: Any, *, board: str, task_id: str) -> dict[str, Any]:
    try:
        result = plugin_api.specify_task_endpoint(
            task_id,
            plugin_api.SpecifyBody(author="dashboard-specify-route-guard-smoke"),
            board=board,
        )
...
def run_smoke(*, checkout: Path, hermes_home: Path, allow_non_disposable: bool) -> dict[str, Any]:
    ...
    normal_id = _create_task(
        plugin_api,
        board=board,
        title="NORMAL-SPECIFY-CONTROL",
        body="ordinary disposable triage task before dashboard specify",
        assignee="ordinary-worker",
    )

    from hermes_cli import kanban_specify  # type: ignore

    specifier_calls: list[str] = []
    original_specify_task = kanban_specify.specify_task

    class FakeOutcome:
        ok = True
        reason = None
        new_title = "NORMAL-SPECIFY-CONTROL-SPECIFIED"

        def __init__(self, task_id: str) -> None:
            self.task_id = task_id

    def fake_specify_task(task_id: str, *, author: str | None = None) -> FakeOutcome:
        specifier_calls.append(task_id)
        conn = kanban_db.connect(board=board)
        try:
            conn.execute(
                "UPDATE tasks SET title = ?, status = ? WHERE id = ?",
                ("NORMAL-SPECIFY-CONTROL-SPECIFIED", "todo", task_id),
            )
            conn.commit()
        finally:
            conn.close()
    ...
    try:
        kanban_specify.specify_task = fake_specify_task
        parent_specify = _specify_attempt(plugin_api, board=board, task_id=parent_id)
        worker_specify = _specify_attempt(plugin_api, board=board, task_id=worker_id)
        normal_specify = _specify_attempt(plugin_api, board=board, task_id=normal_id)
    finally:
        kanban_specify.specify_task = original_specify_task
    ...
    checks = {
        ...
        "dashboard_specify_parent_vfinal_blocked": (
            "PASS"
            if parent_specify["ok"] is False
            and parent_specify["http_status"] == 409
            and parent_after == parent_before
            else "FAIL"
        ),
        "dashboard_specify_worker_subtask_blocked": (
            "PASS"
            if worker_specify["ok"] is False
            and worker_specify["http_status"] == 409
            and worker_after == worker_before
            else "FAIL"
        ),
        "specifier_not_called_for_vfinal_tasks": (
            "PASS" if parent_id not in specifier_calls and worker_id not in specifier_calls else "FAIL"
        ),
        "dashboard_specify_normal_task_allowed": (
            "PASS"
            if normal_specify["ok"] is True
            and normal_after["title"] == "NORMAL-SPECIFY-CONTROL-SPECIFIED"
            and normal_after["status"] == "todo"
            and specifier_calls == [normal_id]
            else "FAIL"
        ),
    ...
    return {
        ...
        "card_ref": {
            "card_id": "VFINAL-HERMES-DASHBOARD-SPECIFY-ROUTE-GUARD-SMOKE",
            "slice_id": "VFINAL_HERMES_DASHBOARD_SPECIFY_ROUTE_GUARD",
            "phase": "F13",
            "risk_effective": "R3",
        ...
        "findings_summary": (
            "Dashboard/API specify-route guard smoke passed: vFinal parent cards "
            "and worker subtasks cannot be rewritten by the generic specifier route, "
            "while ordinary disposable task specify still works through a fake auxiliary specifier."
            if result == "PASS"
            else "Dashboard/API specify-route guard smoke failed; inspect checks."
        ),
        "tool_or_profile": "Hermes v0.16.0 disposable installed runtime with vFinal adapter patches including candidate 0014",
        ...
        "patches": ["adapters/hermes/patches/0014-guard-overkill-vfinal-dashboard-specify-route.patch"],
        "checks": checks,
        "dashboard_specify_evidence": {
            "board_slug": board,
            "parent_task_id": parent_id,
            "worker_task_id": worker_id,
            "normal_task_id": normal_id,
            "worker_count": len(workers),
            "specifier_calls": specifier_calls,
            "attempts": {
                "parent": parent_specify,
                "worker": worker_specify,
                "normal": normal_specify,
            },
            "after": {
                "parent": parent_after,
                "worker": worker_after,
        ...
        "important_limitations": [
            "This proves direct dashboard plugin route functions in a disposable installed Hermes runtime after candidate patch 0014.",
            "The positive normal-task path uses a fake auxiliary specifier to avoid real model calls.",
        ...
        "evidence_refs": [
            ...
            "adapters/hermes/dashboard_specify_route_guard_smoke.py",
            "validation/cards/vfinal-r3-ready.md",
            "validation/hermes-installed-runtime-smoke/dashboard-specify-route-guard-smoke.json",
        ],
        "next_action": "Add patch 0014 to the update sequence and continue proving remaining dashboard/API route families.",
    }
...
def main() -> int:
    ...
    try:
        ...
        result = {
            ...
            "tool_or_profile": "adapters/hermes/dashboard_specify_route_guard_smoke.py",
            "executed_by": "remote-proof-runner",
            "important_limitations": [
